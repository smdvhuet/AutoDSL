...
class CheckForUniqueLoggingIdentifiers extends RuleCheck {
	...
	override check(Rule model) {
		...
		val loggingIdentifiers = model.operations.map[inputs].flatten.map[loggingIdentifier].filter[!empty].toList
		loggingIdentifiers.addAll(model.operations.map[outputs].flatten.map[loggingIdentifier].filter[!empty].toList)
